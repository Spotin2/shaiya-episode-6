...
namespace shaiya
{
    ...
    struct DBAgentSetLocationIncoming
    {
        ...
        DBAgentSetLocationIncoming(ULONG userId, UINT16 mapId, UINT16 direction, SVector& pos)
            : userId(userId), mapId(mapId), direction(direction), x(pos.x), y(pos.y), z(pos.z)
        {
        }

        DBAgentSetLocationIncoming(CUser* user)
            : userId(user->userId), mapId(user->mapId), direction(user->direction), x(user->pos.x), y(user->pos.y), z(user->pos.z)
        {
        }
    };
    #pragma pack(pop)
